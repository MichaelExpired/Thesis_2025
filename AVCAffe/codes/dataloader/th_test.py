import os
import sys
from avcaffe_vid import AVCAffe
import numpy as np
import csv
import cv2 as cv
...
thesis_exp_path = os.path.abspath(os.path.join(script_dir, '../../../'))

python_gaze_tracker_path = os.path.join(thesis_exp_path, 'Python-Gaze-Face-Tracker')
if python_gaze_tracker_path not in sys.path:
    sys.path.append(python_gaze_tracker_path)


try:
    from AngleBuffer import AngleBuffer
    print("AngleBuffer imported successfully!")
except ImportError as e:
    print(f"Failed to import AngleBuffer: {e}")
    sys.exit(1)



avcaffe_path = os.path.join(thesis_exp_path, 'AVCAffe')
if avcaffe_path not in sys.path:
    sys.path.append(avcaffe_path)

ROOT = os.path.join(thesis_exp_path, 'AVCAffe', 'data')

all_clips = []

db_train = AVCAffe(ROOT,
             subset='train',
             return_video=True,
             video_fps=25,
             video_clip_duration=float('inf'),
             return_audio=False,
             return_index=True,
             mode='clip',
             return_labels=True,
...
print('db_train length: ',db_train.__len__)
...
print('db_val length: ',db_val.__len__)

train_val = 'train'


for i in range(0,800):  # Adjust the range as needed, 237480 is total num of samples and is max lim in range() IN TRAINING
    frame_dict = db_train.__getitem__(i)
    par_task = frame_dict.get('pid_task')
    participant = int(par_task)
    task = int(10*round(par_task - participant,1))

    frames = frame_dict.get('frames')
    if frames is None:
        print(f"Frame {i} does not contain image data.")
        continue
    for frame in frames:
        frame_np = np.array(frame)  # Convert PIL image to NumPy array
        all_clips.append([participant,task,frame_np,train_val])
    print("Loaded in training set: ","Participant ",participant,",","Task",task)

train_val = 'val'

for i in range(0,200):  # Adjust the range as needed, 52980 is total num of samples and is max lim in range() IN VALIDATION
    frame_dict = db_val.__getitem__(i)
    ...
    print("Loaded in validation set: ","Participant ",participant,",","Task",task)
...
BLINK_THRESHOLD = 0.3  # Threshold for the eye aspect ratio to trigger a blink
...
def euclidean_distance_3D(points):
    ...
    # Compute vertical distances (upper to lower eyelid)
    vertical1 = np.linalg.norm(P3 - P13)
    vertical2 = np.linalg.norm(P4 - P12)
    vertical3 = np.linalg.norm(P5 - P11)

    # Compute horizontal distance (eye width)
    horizontal = np.linalg.norm(P0 - P8)

    # Compute EAR
    ear = (vertical1 + vertical2 + vertical3) / (3.0 * horizontal)
    
    return ear  # EAR < 0.25 typically means eyes are closed
...
try:
    ...
    with open('biomarkers.csv', mode='w', newline='', encoding='utf-8') as file:
        ...
        writer.writerow(['participant','task','Left Iris Dx', 'Left Iris Dy', 'Right Iris Dx', 'Right Iris Dy','EAR','Pitch','Yaw','Roll','Nose_x','Nose_y','Subset'])  # Write header row

    for i, (par,task, frame,train_val) in enumerate(all_clips):
        #ret, frame = cap.read()
        #frame = cv.imread(all_clips[i])
        ...
        print("Landmarking: ","Participant ",participant,",","Task",task)
        ...
        if results.multi_face_landmarks:
            ...
            """
            # Display the nose direction = gaze direction
            ...
            if eyes_aspect_ratio <= BLINK_THRESHOLD:
                ...
                print("Blink at EAR ratio = ", eyes_aspect_ratio)
                #cv.imshow("Blink", original_frame)
            ...
            if results.multi_face_landmarks is None:
                print(f"No face detected at frame {i}")
            ...
            if PRINT_DATA:
                ...
                """
                with open('eye_pos.csv', mode='a', newline='', encoding='utf-8') as file:
                    writer = csv.writer(file)
                    writer.writerow([par,task,l_dx,l_dy,r_dx,r_dy,eyes_aspect_ratio])
                """
                ...
                if ENABLE_HEAD_POSE:
                    ...
                    if PRINT_DATA:
                        ...
                        print("nose_2D_point: ", nose_2D_point)
            # Logging data
            if LOG_DATA:
        ...
        # Writing the on screen data on the frame
            if SHOW_ON_SCREEN_DATA:
                ...
                if ENABLE_HEAD_POSE:
                    ...
                    cv.putText(frame, f"EAR: {round(eyes_aspect_ratio,3)}", (30, 200), cv.FONT_HERSHEY_DUPLEX, 0.8, (0, 255, 0), 2, cv.LINE_AA)

            with open('biomarkers.csv', mode='a', newline='', encoding='utf-8') as file:
                        writer = csv.writer(file)
                        csv_info = [par,task,l_dx,l_dy,r_dx,r_dy,eyes_aspect_ratio,pitch,yaw,roll,nose_2D_point[0],nose_2D_point[1]]
                        rounded_csv_info = [round(i, 5) for i in csv_info]
                        rounded_csv_info.append(train_val)
                        writer.writerow(rounded_csv_info)
